...
enum Serializer::SerializerCode
Serializer::ToToml(GameObjectContainer &g_obj_cont,
  ...
  g_obj_cont.foreach ([](GameObject *obj) {
    printf("GameObject()\n.uid %lu\n", obj->UID);
    printf(".matrix %f %f %f %f\n", obj->matrix.x, obj->matrix.y,
           obj->matrix.width, obj->matrix.height); // Rectangle
  ...
  icon_cont.foreach ([](TextureIcon *ico) {
    printf("TextureIcon()\n");
    printf(".scale %f %f %f %f\n", ico->scale.x, ico->scale.y, ico->scale.width,
           ico->scale.height);
    printf(".text `%s`\n", ico->text.c_str());
    printf(".fontsize %d\n", ico->fontsize);
    printf(".texture %d\n", ico->texture.id);
