...
Serializer::ToToml(GameObjectContainer &g_obj_cont,
                   IconContainer &icon_cont /*, &ShaderIcon shade_cont*/) {

  auto doc = toml::table{};    // Final TOML document
  auto go_arr = toml::array{}; // List of GameObject items, can reference
                               // tx_arr[] or im_arr[] item indexes
  auto tx_arr = toml::array{}; // List of texture instances and metadata, can
                               // reference im_arr[] item indexes
  auto ic_arr =
      toml::array{}; // List of TextureIcon items, can reference tx_arr items
  int tx_count = 0;
  ...
  { // For each GameObject, capture lambda scope and possibly deduplicate while
    // inserting
    ...
    g_obj_cont.foreach ([&go_arr_ref, &tx_arr_ref, &tex_arr_ref, &im_count_ref,
                         &tx_count_ref](GameObject *obj) {
      auto elm = toml::table{};
      { // .matrix (Rectangle)
      ...
      { // .texture_path and .texture
        ...
        if (obj->texture_path.size() > 0) {
          ...
          // If associated texture instance exists, assume only exists when
          // resource reference exists (texture_path != None)
          if (obj->texture) {
            // At least one Texture{} struct also has position metadata in
            // refrence to the populated texture_list[] resource This is assumed
            // to be a new Texture{} instance, so for now no dedup implemented
            // on these types

            // First, insert texture metadata ref/index into current GameObject
            elm.insert("texture_txid", (*tx_count_ref));

            // Second, insert new texture metadata instance into seperate
            // serialization section for later ref of im_ind
            {
        ...
        if (obj->ObjectListKey) {
          if (obj->ObjectListKey->next) {
            if (obj->ObjectListKey->next->data) {
              // If next object specified, then reference its TOML UID (which
              // may be re-written during TOML injest loading)
  ...
  { // For each TextureIcon, capture lambda scope and possibly deduplicate while
    // inserting
    ...
    auto tex_arr_ref = &texture_list;
    auto tx_arr_ref = &tx_arr;
    icon_cont.foreach ([&ic_arr_ref, &im_count_ref, &tx_count_ref, &tex_arr_ref,
                        &tx_arr_ref](TextureIcon *ico) {
      auto elm = toml::table{};
      ...
      { // .text and .texture
        ...
        if (ico->text.size() > 0) {
          ...
          { // Handle inserting/deduplicating texture resources
            ...
            // convert im_ind implicitly just to please the compiler
            if (*(u64 *)&im_ind < tex_arr_ref->size()) {
              // Reference texture resource index
              elm.insert("texture_imid", im_ind);
              (*tex_arr_ref)[im_ind].tid_arr.push_back(im_ind);
            } else {
          ...
          // If associated texture instance exists, assume only exists when
          // resource reference exists (text != None)
          // added the cast to get compiler to stop bitching
          if ((void *)&ico->texture) { // Always true as in contained
            // At least one Texture{} struct also has position metadata in
            // refrence to the populated texture_list[] resource This is assumed
            // to be a new Texture{} instance, so for now no dedup implemented
            // on these types
        ...
        if (ico->ref) {
          if (ico->ref->next) {
            if (ico->ref->next->data) {
              ...
              // Does not have .UID like GameObject, so for (debug) info show
              // next texture resource location contained by linked list
  ...
  { // For each possibly deduplicated resource (texture_list/im_arr) insert into
    // seralization queue now
    int count = 0;
  ...
  // Be nice and list/serialize raw external resources first before anything
  // that might reference them But the toml++ serializer doesn't respect
  // insertion order???
  // ...
